...
class Requests:
    ...
    def copy_run_update_script(link):
        try:
            ...
            pass
        shutil.copyfile("updatescript.bat", os.path.join(os.getenv('APPDATA'), "decloak", "updatescript.bat"))
        r_zip = requests.get(link, stream=True)
        z = zipfile.ZipFile(io.BytesIO(r_zip.content))
        z.extractall(os.path.join(os.getenv('APPDATA'), "decloak"))
        subprocess.Popen([os.path.join(os.getenv('APPDATA'), "decloak", "updatescript.bat"), os.path.join(os.getenv('APPDATA'), "decloak", ".".join(os.path.basename(link).split(".")[:-1])), os.getcwd(), os.path.join(os.getenv('APPDATA'), "decloak")])
